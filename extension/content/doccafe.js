/* DocCafe floating "Log this search" button. No unhandled throws. */
(function () {
  'use strict';
  ...
  function parsed() {
    try {
      return RB.parseDocCafeSearch(window.location.href, document.title);
    } catch {
      return { keywords: '' };
    }
  }
  ...
  function onClick() {
    ...
    try {
      ...
      const payload = RB.buildDocCafePayload(
        window.location.href,
        document.title,
        RB.extractDocCafeCount(document, document.title),
        Date.now(),
      );
  ...
  function tick() {
    try {
      ...
      if (btn) btn.style.display = parsed().keywords ? '' : 'none';
    } catch {
      /* ignore */
    }
  }
